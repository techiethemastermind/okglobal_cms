	...
	$.fn.countdown.begin = function (parent, settings) {
		...
		if (!settings.init) {
			...
			$.each(timespan, function (k, v) {
				...
				var container, time, label;

				// Create elements container
				container = $('<h1/>').attr('id', k);

				// Create time element
				time = $('<label/>').addClass('tick').text(v < 10 ? '0' + v : v.toLocaleString());
				...
				if (settings.labels) {
					...
					container.append(time).append(label);
				} else {
			...
			// Update each element
			$.each(timespan, function (k, v) {
				$('.tick', '#' + k).text(v < 10 ? '0' + v : v.toLocaleString());
